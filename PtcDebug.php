...
class PtcDebug {
    ...
    public static function addCss($css) {
        static::$_panelCss = static::$_panelCss . "\n" . $css;
    }
    ...
    public static function tickHandler() {
        //$now = microtime( true );
        if (static::$_codeCoverage || static::$_functionTrace) {
            $bt = debug_backtrace();
        }
        ...
        if (static::$_codeCoverage) {
            static::_codeCoverageAnalysis($bt);
        }
        if (static::$_functionTrace) {
            static::_traceFunctionCalls($bt);
        }
    ...
    public static function stopTimer($reference = null, $precision = 1) {
        ...
        if (static::$_options['debug_console']) {
            static::$_buffer[$last['key']]['console_time'] = static::$_buffer[$last['key']]['time'];
        }
        return true;
    }
    ...
    public static function convertMemUsage($val, $precision = 2) {
        ...
        return $usage = ( $val ) ? @round($val / pow(1024, ($i = floor(log($val, 1024)))), $precision) . $ram[$i] : '0 Bytes';
    }
    ...
    public static function errorHandler($errno, $errstr, $errfile, $errline) {
        ...
        if (static::$_options['die_on_error'] && static::_msgType($errno) == 'Php Error') {
            die();
        }
                ...
                if ($file_lines) {
                    ...
                    foreach ($file_lines as $line) {
                        ...
                        if ($search_count > 0) { // we found matches 
                            $line = preg_replace('|' . $query . '|', '<span style="color:yellow;font-weight:bold;">' . $query .
            ...
            if (@$arr['console_string'] || @$arr['console_statement']) {
                ...
                if (static::$_options['show_' . $key]) {
                    if ('error' === $arr['console_statement']) {
                        $handler->handleError($arr['console_category'], $arr['errstr'], $arr['errfile'][0], $arr['errline'][0], null, 2);
                    } else {
    ...
    protected static function _checkAccess($allowedIps = null) {
        ...
        if (static::$_options['allowed_ips']) {
            ...
            if (@in_array(@$_SERVER['REMOTE_ADDR'], static::$_options['allowed_ips'])) {
                return true;
            }
            return false;
        }
    ...
    protected static function _buildBuffer($type, $string, $statement = null, $category = null) {
        if (@in_array($category, static::$_options['exclude_categories'])) {
            return;
        }
        if (defined('_PTCDEBUG_NAMESPACE_') &&
            ...
            if ($string === '{errorHandler}') {
                ...
                if ($statement['errfile'] == 'trace') {
                    ...
                    if (!@is_array($buffer['errline'])) {
                        $buffer['errline'] = array();
                    }
                    if (!@is_array($buffer['errfile'])) {
                        $buffer['errfile'] = array();
                    }
                    if (!@is_array($buffer['function'])) {
                        $buffer['function'] = array();
                    }
                    if (!@is_array($buffer['class'])) {
                        $buffer['class'] = array();
                    }
                ...
                if (!$category) {
                    $category = static::$_options['default_category'];
                }
            ...
            foreach (static::$_watchedVars as $variableName => $variableValue) {
                if (is_array($variableValue)) {
                    ...
                    if (@$var !== @$variableValue['value']) {
                        ...
                        static::$_watchedVars[$variableName] = array('value' => $var, 'callback' => $variableValue['callback']);
                    }
                } else {
    ...
    protected static function _codeCoverageAnalysis($backtrace = null) {
        ...
        foreach ($backtrace as $k => $v) {
            if (@$v['file'] == __FILE__ ||
                    @strpos($v['file'], 'runtime-created function')) {
                continue;
            }
            if (@$v['line'] && !@array_key_exists(@$v['line'], @static::$_coverageData[$v['file']])) {
    ...
    protected static function _formatVar($var) {
        if (is_array($var) || is_object($var)) {
            $html_string = static::_doDump($var);
        } else if (@is_bool($var)) {
            $html_string = '<span style="color:#92008d;">' . ($var == 1 ? 'TRUE' : 'FALSE') . '</span>';
        } else if (@is_null($var)) {
            $html_strisng = '<span style="color:black;">NULL</span>';
        } else if (@is_float($var)) {
            $html_string = '<span style="color:#10C500;">' . $var . '</span>';
        } else if (is_int($var)) {
            $html_string = '<span style="color:red;">' . $var . '</span>';
        }
        // could be a string
        else {
            $html_string = '<span>' . static::_cleanBuffer(@print_r($var, true)) . '</span>';
        }
        return @$html_string;
    }
    ...
    protected static function _doDump(&$var, $varName = NULL, $indent = NULL, $reference = NULL, $depth = 0) {
        ...
        if ((is_array($var) && isset($var[$keyvar])) || is_object($var) && property_exists($var, $keyvar)) {
            ...
            switch ($type) {
                case 'String':$type_color = '<span style="color:#CF3F33;">';
                    break;
                case 'Integer':$type_color = '<span style="color:red;">';
                    break;
                case 'Double':$type_color = '<span style="color:#10C500;">';
                    $type = 'Float';
                    break;
                case 'Boolean':$type_color = '<span style="color:#92008d;">';
                    break;
                case 'NULL':$type_color = '<span style="color:black;">';
                    break;
            ...
            if (is_array($avar)) {
                ...
                if (( @$rf->getName() == '{closure}')) { // work with lambda functions first
                    ...
                    if (@$rf->getFileName()) {
                        $result .= @$rf->getFileName();
                    }
                    if (@$rf->getStartLine()) {
                        $result .= ':' . @$rf->getStartLine();
                    }
                    if (@$rf->getStartLine()) {
                        $result .= '-' . @$rf->getEndline();
                    }
                    $result .= '</span><br>';
                } else {
                    ...
                    if ($depth < static::$_options['max_dump_depth']) {
                        ...
                        if ($methods) {
                            ...
                            $result .= static::_doDump($class_methods, '<span style="color:#CF7F18;">[**class_methods:' . get_class($avar) .
                ...
                if ($varName == "recursion_protection_scheme") {
                    return;
                }
                ...
                if (is_bool($avar)) {
                    ...
                    $result .= strlen($avar) . ')</span> ' . $type_color . 'NULL</span><br>';
    ...
    protected static function _debugTrace($depth = NULL) {
        if (!$depth) {
            $depth = static::$_options['trace_depth'];
        }
        if (version_compare(PHP_VERSION, '5.3.6') < 0) {
            $raw_trace = debug_backtrace(false);
        } else {
            $raw_trace = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS);
        }
        ...
        if (!empty($raw_trace)) {
            ...
            if ($depth > count($raw_trace)) {
                $depth = count($raw_trace);
            }
            for ($i = 0; $i < $depth; $i++) {
    ...
    protected static function _buildMenu() {
        ...
        $ul .= '<li>' . static::_menuLinks('msgPanel', 'log & messages ( ' . $num_msg . ' ) ', 'messages ( ' . $num_msg . ' ) ') . '</li>';
        $num_msg = @count(static::$_buffer['sql']);
        $ul .= '<li>' . static::_menuLinks('sqlPanel', 'mysql query messages ( ' . $num_msg . ' ) ', 'sql ( ' . $num_msg . ' ) ') . '</li>';
        $num_msg = (@count(static::$_finalCoverageData) + @count(static::$_finalTraceData) );
        $ul .= '<li>' . static::_menuLinks('analysisPanel', 'analysis ( ' . $num_msg . ' ) ', 'analysis ( ' . $num_msg . ' ) ') . '</li>';
        $ul .= '<li>' . static::_menuLinks('varsPanel', 'configuration & environment variables', 'vars & config') . '</li>';
        ...
        if (@static::$_buffer[$type]) {
            ...
            foreach (static::$_buffer[$type] as $k => $arr) {
                ...
                if ($type == 'log') {
                    $div .= '<td class="fixed">';
                    switch (@$arr['var_type']) {
                        case 'boolean' : $color = 'color:#92008d;';
                            break;
                        case 'NULL' : $color = 'color:black;';
                            break;
                        case 'integer' : $color = 'color:red;';
                            break;
                        case 'double' : $color = 'color:#10C500;';
                            break;
                        case 'array' : $color = 'color:blue';
                            break;
                        case 'object' : $color = 'color:#CF3F33';
                            break;
            ...
            $div = '<span class="vars">no messages</span>';
        }
        return $div;
    }
    ...
    protected static function _buildVarsPanel() {
        ...
        if (!@empty($included_files)) {
            ...
            $div .= '<span class="vars">Could not get included files!</span>';
        }
        $div .= '</div>';
        ...
        $div .= static::_buildInnerVars('functionsInternal', 'Internal Functions', @$functions['internal']);
        $div .= static::_buildInnerVars('functionsUser', 'User Functions', @$functions['user']);
        $div .= static::_buildInnerVars('declared_classes', 'Declared Classes', $classes = @get_declared_classes());
        $div .= static::_buildInnerVars('declared_interfaces', 'Declared Interfaces', $interfaces = @get_declared_interfaces());
        ...
        if (!static::$_options['show_globals']) {
            ...
            $div .= static::_buildInnerVars('globals', 'Globals ', array_reverse($GLOBALS));
        }
        return $div .= '</div>';
    }
    ...
    protected static function _buildW3cPanel() {
        ...
        if (static::$_options['show_w3c']) {
            ...
            $div .= '<span class="vars">Panel is Disabled</span>';
        }
        $div .= '</div>';
        return $div = static::_compressHtml($div);
    }
    ...
    protected static function _includeJs() {
        return static::_compressHtml(
					...
					function ptc_read_code(filename,line) 
					{
                        ...
                        $path = addslashes(str_replace(realpath($_SERVER['DOCUMENT_ROOT']), '', realpath(dirname(__FILE__)))) . '/PtcDebug.php?ptc_read_file="+filename;
					...
					function ptc_search_string( )
					{
						...
						{
							var query="http://' . addslashes($_SERVER['HTTP_HOST']) .
                        $path = addslashes(str_replace(realpath($_SERVER['DOCUMENT_ROOT']), '', realpath(dirname(__FILE__)))) .
...
if (@$_GET['ptc_read_file']) {
    @session_start();
    if (!@$_SESSION['ptcdebug']['code_highlighter']) {
        exit();
    }
...
if (@$_GET['ptc_search_files']) {
    @session_start();
    if (!@$_SESSION['ptcdebug']['search_files']) {
        exit();
    }
